function [rectx,recty,area,perimeter,maxc,minc,theta] = minboundrect(x,y,metric)
...
dist2 = sqrt((rectx(3)-rectx(2))^2 +(recty(3)-recty(2))^2);

minc = min([dist1 dist2]);
maxc = max([dist1 dist2]);

v0 =[1 0 0];
if dist1>dist2
    v = [rectx(2)-rectx(1) recty(2)-recty(1) 0];
    
else
    v = [rectx(3)-rectx(2) recty(3)-recty(2) 0];
end
theta = rad2deg(atan2(norm(cross(v0,v)),dot(v0,v)));
